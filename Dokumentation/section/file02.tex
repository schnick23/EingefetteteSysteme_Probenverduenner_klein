...
Sollten allerdings Fehler auftreten können diese, wie in \autoref{Fehlerfaelle} beschrieben, behoben werden. Außer derjenige Fehler ist nicht in diesem Dokument behandelt, dann sollte sich an die Entwickler des Projektes gewendet werden.
...
Es dürfen nur Chemikalien verwendet werden, die mit den Materialien des Gerätes kompatibel sind (vgl. \autoref{Materialien}) 

\subsection{Elektrische Sicherheitshinweise}\label{ElektrischeSicherheitshinweise}
Der Pinkler wird über ein 230V Netzteil angeschlossen und mit Strom versorgt. Nahezu die komplette Elektronik des Geräts befindet sich auf der Unterseite der Bodenplatte. 
...
Nachdem dieser gedrückt wurde fährt sich sämtliche Elektronik des Geräts runter und alles bleibt dort stehen wo es ist. Um das Gerät nach dem Notfall wieder einzuschalten, muss der Not-Aus-Knopf einmal in die Richtung gedreht werden, die auf dem Knopf selber beschrieben ist.\\

Danach fährt das Gerät wieder hoch und der Zugriff auf die grafische Oberfläche ist möglich. Dort findet man den Button \textbf{Reset}, durch welchen der Probenverdünner zurück an die Nullpositionen fährt und alles resetet. Erst danach kann das Gerät wieder normal genutzt werden.\\

\begin{figure}[H] % r = rechts, l = links
    \centering
    \includegraphics[width=0.8\textwidth]{images/Startseite.png}
    \caption{Startseite des Probenverdünners}
    \label{start}
\end{figure}

\subsection{Mechanische Sicherheitshinweise}\label{MechanischeSicherheitshinweise}
Aus Kostengründen wurden die Füße sowie die Steckerhalter für Netzwerk und Strom 3D-gedruckt. Diese sind zwar stabil aber können bei unvorsichtiger Benutzung brechen. Sollte dies auftreten, können die nötigen Pläne, um diese erneut zu drucken, sind in GitHub unter dem Ordner \textit{3D-Drucke} zu finden. 
...
Während des gesamten Durchlaufs darf nichts in dem Pinkler landen. Insbesondere die Linearführung des Spritzkopfes und der Hubtisch birgt eine Quetschgefahr. Außerdem sollten lange Haare, Kleidung oder Schmuckstücke von den beweglichen Teilen ferngehalten werden, um ein Verfangen zu vermeiden. Besonders zu beachten ist wieder der Linearführung des Spritzkopfes.

Zudem ist der Probenverdünner nicht für hohe Lasten geeignet. Das Gerät sollte nicht als Ablage oder Stütze genutzt werden.
